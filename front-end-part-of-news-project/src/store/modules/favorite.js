...
export const useFavoriteStore = defineStore('favorite', {
  ...
  getters: {
    ...
    isFavorite: (state) => (newsId) => state.favorites.some(item => (item.newsId === newsId || item.id === newsId)),
  },
  ...
  actions: {
    ...
    // 取消收藏 - 本地
    removeFavorite(id) {
      // 同时检查记录ID (id) 和新闻ID (newsId)
      this.favorites = this.favorites.filter(item => item.id !== id && item.newsId !== id);
      this.saveFavorites();
    },

...
onMounted(async () => {
  ...
  try {
    ...
    // 确保关闭加载提示
    showToast.clear();
    showToast.fail(t('profile.getUserInfoFailed'));
  }
});
...
const showPasswordConfirm = () => {
  ...
  showDialog({
    ...
    if (!oldPassword.value) {
      showToast(t('profile.enterCurrentPassword'));
      return;
    }

    if (!newPassword.value) {
      showToast(t('profile.enterNewPassword'));
      return;
    }
    ...
    try {
      // 显示加载提示
      const loadingInstance = showLoadingToast({
        message: t('profile.changingPassword'),
        forbidClick: true,
        duration: 0
      });
      ...
      showToast.clear();
      showToast.fail(t('profile.passwordChangeFailed'));
    }
  }).catch(() => {
    // 点击取消按钮
  });
...
const showBioDialog = () => {
  ...
  showDialog({
    ...
    message: h('div', { style: 'text-align: left; padding: 10px 0;' }, [
      h('div', { style: 'margin-bottom: 15px;' }, [
        h('div', { style: 'margin-bottom: 5px; text-align: left;' }, `${t('profile.bio')}:`),
        h('textarea', {
    ...
    try {
      ...
      showToast.clear();
      showToast.fail(t('profile.bioUpdateFailed'));
    }
  }).catch(() => {
    // 点击取消按钮
  });

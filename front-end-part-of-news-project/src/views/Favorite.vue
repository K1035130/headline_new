  ...
  <div class="favorite-container">
    ...
    <div class="favorite-list" v-if="favoriteStore.getFavorites.length">
      <div class="favorite-item" v-for="item in favoriteStore.getFavorites" :key="item.id">
        <van-cell @click="goToNewsDetail(item.newsId )" :border="false">
          <template #title>
            <div class="news-item">
        ...
        <van-button 
          ...
          size="mini" 
          icon="cross"
          @click="confirmDelete(item.newsId )"
        ></van-button>
      </div>
    </div>
...
const removeFavorite = async (newsId) => {
  const result = await favoriteStore.removeFavoriteApi(newsId);
  if (result.success) {
    // API请求成功后，更新本地收藏列表
    favoriteStore.removeFavorite(newsId);
  }
};
